...
public class BoardController {
    ...
    public String save(@ModelAttribute MemberDTO memberDTO, PasswordEncoder passwordEncoder) {
        memberService.save(memberDTO,passwordEncoder);
        return "login";
    }

    @PostMapping("/member/login")
    public String login(@ModelAttribute("member") MemberDTO memberDTO,PasswordEncoder passwordEncoder, HttpSession session, Model model, @PageableDefault(page = 1) Pageable pageable) {
        MemberDTO loginResult = memberService.login(memberDTO,passwordEncoder);
        if (loginResult != null) {
    ...
    public String update(@ModelAttribute MemberDTO memberDTO, PasswordEncoder passwordEncoder) {
        memberService.update(memberDTO, passwordEncoder);
        return "redirect:/member/" + memberDTO.getId();
    }
